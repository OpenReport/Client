...
$app->get("/:apiKey/:id", function ($apiKey, $id) use ($app, $response) {
    ...
    $eventForm = Record::find($id);
    // package the data
    $response['data'] = $eventForm->values_for(array('id', 'task_id','meta'));
    $response['count'] = 1;
    // send the data
    echo json_encode($response);
...
$app->post('/', function () use ($app, $response) {
 ...
 *
 */
$app->get("/report/:apiKey/:formId", function ($apiKey, $formId) use ($app, $response) {
...
/**
 * Fetch Report Data for Tasks
 *
 * get: /report/task/{id}
 *
 */
//$app->get("/task/:id", function ($id) use ($app, $response) {
//
//    // find all event records
//    $eventData = Record::find('all', array('conditions' => array('task_id = ?', $id)));
//    // normalize data
//    $data = array();
//    foreach($eventData as $record){
//        $temp = $record->meta;
